...
export function parsePath(source) {
...
function parseSinglePath(source) {
    const segments = [];
    let index = 0;
...
function readRecursiveSelector(source, startIndex) {
    if (startIndex >= source.length) {
        return { node: { type: "Recursive" }, nextIndex: startIndex };
    }
    const char = source[startIndex];
    if (char === "[") {
        const result = parseBracketExpression(source, startIndex);
        if (result.node.type === "Child") {
            const node = { type: "Recursive", selector: result.node.selector };
    ...
    if (char === "*") {
        const node = { type: "Recursive", selector: { type: "WildcardSelector" } };
...
function readChildSelector(source, startIndex) {
    if (startIndex >= source.length) {
...
function readTypeSelector(source, startIndex) {
    ...
    const node = { type: "TypeSelector", name: ident.value };
    return { node, nextIndex: ident.end + 2 };
}
function readEscapedName(source, startIndex) {
